...
class OCRGradeVerifier:
        ...
        def _extract_with_vision(self, image_path: str) -> dict:
            """
            Try to extract grades and name using Google Vision API (lightweight fallback).
            Returns a dict with 'grades', 'student_name', and 'full_text'.
            """
            try:
                ...
                # Simple parsing: look for subject grades and name in text
                grades = {}
                student_name = None
                lines = full_text.splitlines()
                for line in lines:
                    # Try to find name
                    if not student_name and 'Name:' in line:
                    ...
                    for subject in self.SUBJECTS:
                        if subject in line:
                            ...
                            if match:
                                grades[subject] = float(match.group(1))
                return {
                    'grades': grades,
                ...
                print(f"Vision API fallback failed: {e}")
                return {}
        ...
        def extract_grades_and_name_from_image(self, image_path: str) -> Dict:
            ...
            try:
                ...
                # If Vision API found all subjects and a name, use it
                if vision_result and len(vision_result.get('grades', {})) >= 6 and vision_result.get('student_name'):
                    print("✓ Vision API succeeded, skipping Gemini.")
                    validated_grades = self._validate_grades(vision_result['grades'])
                    return {
                        'grades': validated_grades,
                        'student_name': vision_result['student_name'],
                        'full_text': vision_result['full_text']
                    }
                # 2. Fallback to Gemini if Vision API is incomplete
                print("[1] Loading image...")
                img = Image.open(image_path)
    ...
    Note: Use \"GMRC\" as the key if the card shows \"GMRC\" instead of \"EsP\"."""
        ...
        def verify_student_name(self, extracted_name: str, registered_full_name: str) -> Dict:
            """
            Verify if the extracted name matches the registered full name.
            
            Args:
                extracted_name: Name extracted from the report card
                registered_full_name: Correct name for verification
        ...
        def _check_middle_name_match(self, middle1: str, middle2: str) -> bool:
            """
            Check if two middle names match. Handles full names vs initials.
