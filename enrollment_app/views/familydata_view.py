...
def family_data_form(request):
    """
    Handle family data form.
    After completion, routing depends on enrollment type:
      - new:        → non_academic survey
      - old:        → enrollment_complete (no survey/academic/ML)
      - transferee: → document_submission
    """
    ...
    # Determine enrollment type for routing
    enrollment_type = (
    ...
    if request.method == 'POST':
        ...
        # ============================================================
        # ROUTING BASED ON ENROLLMENT TYPE
        # ============================================================
        if enrollment_type == 'old':
            # Old students skip survey & academic — go straight to completion
            messages.success(request, 'Family data saved. Your enrollment has been submitted!')
            return redirect('enrollment_app:enrollment_complete_old')

        elif enrollment_type == 'transferee':
            # Transferees skip survey & academic — go to document submission
