...
HOW TO USE:
    1. Make sure you have already run spark_training.py first.
       It saves trained models to the spark_models/ folder.
    2. Run this script:  python3 spark_predict.py
    3. Answer the prompts to enter a new student's data.
    4. The system will:
        - Check STE eligibility based on Grade 6 Math, Science, English
        - Loop through all 5 programs and predict Grade 7 grades
        ...
        - Generate a visual report saved as a PNG file

REQUIREMENTS:
    - spark_models/ folder must exist (created by spark_training.py)
...
def load_models():
    """Load all trained models and config from disk."""
    required = [
    ...
    for f in required:
        if not os.path.exists(os.path.join(MODELS_DIR, f)):
            ...
            print("     Please run spark_training.py first to train and save models.")
            sys.exit(1)
...
# Each question has: key, label, type, options/range, help text
# ==============================================================================
...
def build_questions():
    ...
    input_type:
        'int'   — integer within range
        'float' — decimal within range
        'choice'— must pick from option list
        'yesno' — 0 or 1
    """
    questions = [
        # ── BASIC INFO ────────────────────────────────────────────────────────
...
def predict_for_student(student_data, config, classifier, regressors):
    """
    Runs the 5-loop prediction process for one student.
    Returns a results dict with predicted grades and suitability per program.
    """
    ...
    # Add computed values not directly asked
    ...
    # Build DataFrame row with all expected features
    feat_row = {}
    for f in FEATURES:
        feat_row[f] = student_data.get(f, 0)   # default 0 for any missing feature
    ...
    # ── 5-LOOP: Predict Grade 7 grades in each program ─────────────────────
    program_results = {}
    pred_avgs_for_clf = {}
    ...
    for p, name in PROGRAM_MAP.items():
        ...
        predicted_final_avg = float(model.predict(X_imputed)[0])
        predicted_final_avg = round(min(max(predicted_final_avg, 60), 100), 2)

        # Estimate per-subject grades: scale from group mean proportionally
        # (regression model predicts final avg; per-subject is an approximation)
        # We use the ratio of predicted avg to training mean to scale subjects
        subject_preds = {}
        for subj in subjects:
            # Use the same model to estimate — in a full multi-output setup
            # each subject would have its own model. Here we approximate by
            # applying a proportional offset from the predicted final average.
            subject_preds[subj] = round(predicted_final_avg, 2)

        # Suitability check
        grade_threshold_met = predicted_final_avg >= SUITABILITY_THRESHOLD[p]
        if p == 1:   # STE requires both grade threshold AND G6 eligibility
    ...
    # ── Classification: overall program recommendation ──────────────────────
    ...
    proba_dict = {int(c): round(float(p), 4) for c, p in zip(clf_classes, clf_proba)}
...
def display_results_console(result):
    """Print a detailed recommendation to the terminal."""
    ...
    print("\n" + "=" * 66)
    print(f"  SPARK RECOMMENDATION REPORT")
    ...
    if result['ste_eligible']:
        print("✅ ELIGIBLE (Math, Science, English all ≥ "
    ...
    for rank, p in enumerate(result['ranked'], 1):
        ...
        print(f"  {name_p:<12} {avg:>10.2f} {thr:>10} "
    ...
    for i, p in enumerate(top3):
        ...
        margin_str = (f"+{r['margin']:.2f} above threshold"
                      if r['margin'] >= 0 else f"{r['margin']:.2f} below threshold")
        print(f"      Margin                       : {margin_str}")

        # Explanation
        if p == 1:
            if result['ste_eligible']:
                print(f"      Why suitable: Student meets all Grade 6 STE eligibility "
                      f"requirements and predicted Grade 7 average of {r['predicted_avg']:.2f} "
                      f"meets the {r['threshold']} threshold.")
            else:
                print(f"      ⚠  Note: STE is in Top 3 by predicted grade "
                      f"but student does NOT meet G6 eligibility requirements.")
                for reason in result['ste_failed']:
                    print(f"         ✗ {reason}")
        else:
            if r['is_suitable']:
                print(f"      Why suitable: Predicted Grade 7 average of "
                      f"{r['predicted_avg']:.2f} meets the {r['threshold']} threshold "
                      f"for {r['name']}.")
            else:
    ...
    if has_pref and pref in [1,2,3,4,5]:
        ...
        if pref in top3:
            ...
            if pref == 1 and not result['ste_eligible']:
                print(f"      Primary reason: Grade 6 eligibility requirement not met.")
                for reason in result['ste_failed']:
                    print(f"        ✗ {reason}")
                print(f"      The student must have Grade 6 Math, Science, AND English ≥ "
                      f"{result['STE_ELIGIBILITY_MIN_GRADE']} to qualify for STE.")
                ...
                print(f"      The system recommends a program where the student is "
                      f"predicted to perform at or above the required standard.")
    else:
    ...
    print(f"\n  {'─' * 62}")
    print(f"  Classification Model Prediction : {PROGRAM_MAP[result['clf_pred']]}")
    print(f"  (This is the model's direct classification — may differ from")
    print(f"   grade-based Top 1 when borderline cases exist)")
    print(f"  {'─' * 62}\n")
...
def generate_visual_report(result, output_filename=None):
    """
    Creates a comprehensive visual recommendation report as a PNG.
    ...
    # ── Title ────────────────────────────────────────────────────────────────
    fig.text(0.5, 0.97,
             f'SPARK System — Program Recommendation Report',
    ...
    # ── Plot A: Predicted Grade 7 Averages per Program (bar chart) ──────────
    ...
    bar_colors       = [
        SUITABLE_COLOR if prog_res[p]['is_suitable'] else UNSUITABLE_COLOR
        for p in ranked
    ]
    ...
    for bar, val, p in zip(bars, preds_ordered, ranked):
        suitable_tag = '✅' if prog_res[p]['is_suitable'] else '❌'
        ax_a.text(bar.get_x() + bar.get_width()/2.,
                  bar.get_height() + 0.3,
                  f'{val:.2f} {suitable_tag}',
                  ha='center', va='bottom',
    ...
    # ── Plot B: Grade 6 Subject Breakdown ────────────────────────────────────
    ...
    for subj, val in g6_subjects.items():
        if subj in STE_SUBJECTS:
            g6_colors.append('#2E86AB' if val >= 83 else '#E63946')
        else:
            g6_colors.append('#6C757D')
    ...
    # ── Plot C: Classification Probability ───────────────────────────────────
    ...
    ax_e.set_title('Top 3 Recommended Programs', fontsize=10, fontweight='bold',
                   pad=8)
    ax_e.set_xlim(0, 1)
    ax_e.set_ylim(0, 1)
    ...
    if has_pref and pref in [1,2,3,4,5]:
        ...
        if not in_top3:
            ...
            ax_f.text(0.5, 0.18,
                      ...
                      f"with a predicted average of "
                      f"{prog_res[top3[0]]['predicted_avg']:.2f}.",
                      transform=ax_f.transAxes,
    ...
    if output_filename is None:
        output_filename = f'recommendation_{safe_name}.png'
...
def main():
    ...
    print(f"  ✓ Programs: {', '.join(config['PROGRAM_MAP'].values())}")

    while True:
        # Collect student data through interactive prompts
        student_data = collect_student_data()

        print("\n  Processing... predicting Grade 7 performance across all 5 programs...")
        ...
        # Ask if user wants to test another student
        print("\n" + "─" * 66)
